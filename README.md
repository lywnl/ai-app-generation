...
实时对话中的文件操作、`readSkill` 与 `buildProject` 使用紧凑工具卡片，详情默认展开，可手动收起，执行结束后原位置更新真实结果，不再切换为另一份 Markdown 代码。代码区高度随内容自然展开，页面整体负责纵向滚动；长行保留原始字符并允许代码区横向滚动。本次会话重新订阅时保留展开状态和修改前后选择。写入与修改代码随参数增量更新，仅改变视觉排版，不修改原始代码；无代码时生成中显示“正在生成代码…”，工具完成或回合结束后显示“暂无代码内容”。构建卡片保留真实构建次数、阶段、失败摘要与取消状态，不重复展示正文中的工具选择和构建结果。读取类工具只显示经过过滤的结果说明，普通只读问答结束仍按原规则隐藏文件读取卡片。该优化仅作用于实时会话，整页刷新后的数据库历史仍使用原 Markdown 正文，无需迁移数据。

`modifyFile` 的“修改前／修改后”表示本次替换片段，默认显示修改后。参数生成期间保留原文；调用结束后自动判断当前侧：合法的单个字符串（单双引号或无插值模板，可带一个尾逗号）解码一层后若含换行，直接展开内部内容；不能安全展开时显示原始片段。界面不再提供“源码／内容预览”切换。完整 Markdown 代码围栏只提取内部代码并按语言高亮，其他解码内容按文本展示，不执行脚本或加载图片、不修补非法字符串。原始片段保留语法高亮与字符差异，红色表示删除、绿色表示新增；展开后的内容不叠加源码差异，差异不代表实际修改成功。

预览与差异仅在展开且工具调用结束后计算，同内容复用计算结果。单侧超过 64 KiB 时保留完整原文，字符差异计算超过 50ms 时关闭差异标记。该功能使用 `acorn`、`diff` 与现有 `highlight.js`，不格式化或改写工具参数、实际文件及历史正文。
