...
实时对话中的文件操作、`readSkill` 与 `buildProject` 使用紧凑工具卡片，详情默认展开，可手动收起，执行结束后原位置更新真实结果，不再切换为另一份 Markdown 代码。展开的代码区桌面端固定 240px、移动端固定 180px；本次会话重新订阅时保留展开状态和修改前后选择。写入与修改代码随参数增量更新，长行在代码区内自动换行，仅改变视觉排版，不修改原始代码；无代码时生成中显示“正在生成代码…”，工具完成或回合结束后显示“暂无代码内容”。构建卡片保留真实构建次数、阶段、失败摘要与取消状态，不重复展示正文中的工具选择和构建结果。读取类工具只显示经过过滤的结果说明，普通只读问答结束仍按原规则隐藏文件读取卡片。该优化仅作用于实时会话，整页刷新后的数据库历史仍使用原 Markdown 正文，无需迁移数据。
