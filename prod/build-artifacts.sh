...
rm -rf "$PROD/artifacts/frontend/dist" "$PROD/sql/migrations" "$PROD/embed_text"
...
cp -R "$ROOT/sql/migrations" "$PROD/sql/migrations"
...
(cd "$PROD" && find artifacts/frontend artifacts/backend sql embed_text -type f -print0 | sort -z | xargs -0 shasum -a 256) > "$PROD/artifacts/SHA256SUMS"
echo "构建完成，产物位于 $PROD/artifacts"
