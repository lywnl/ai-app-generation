...
- [ ] 增加逐字符分片测试，验证 `[工具调用]` 标记跨多个分片时不会部分泄漏。
- [ ] 增加“可信前缀 + 单个伪工具块”测试，验证只输出可信前缀，流结束时确认协议退化。
- [ ] 增加两个参数不同的伪工具块测试，验证两个块及中间未受信正文均不输出。
- [ ] 增加未知工具、错误 JSON、重复 JSON 字段和未闭合 JSON 测试，验证明确工具标记不会被原样释放。
- [ ] 增加正常 Markdown、普通 JSON 和仅提及 `tool_calls` 的技术说明测试，验证没有明确工具标记时不误杀。
- [ ] 增加单个伪工具块流结束后启动第二次模型请求的测试。
- [ ] 保留两个规范指纹相同的伪工具块提前启动恢复的测试。
- [ ] 增加纠正 generation 再次退化测试，断言严格只有两次模型请求并以 `PROTOCOL_ERROR` 结束。
- [ ] 运行红灯测试：
...
- [ ] 继续保留可能构成 `[工具调用]` 的跨分片标记前缀，避免标记字符提前输出。
- [ ] 一旦确认完整 `[工具调用]` 标记，进入该 generation 的隔离状态。
- [ ] 隔离后只允许返回标记之前的可信前缀，不再释放标记、工具名、参数及其后续正文。
- [ ] 对已注册工具和合法 JSON 继续生成规范化指纹，用于重复块提前熔断。
- [ ] 未注册工具、坏 JSON、残缺 JSON 和不同参数候选保持隔离，在流结束时返回 `STREAM_FINISHED`。
- [ ] 收到真实结构化工具通知时，将候选标记为需要丢弃，但不释放被隔离正文。
- [ ] 已出现真实结构化工具后仍继续检测后续普通正文，防止混合响应中的伪工具文本绕过检测。
- [ ] 设置 `65_536` 字符隔离上限；达到上限返回 `QUARANTINE_LIMIT`，避免异常输出无限占用内存。
- [ ] 检测器只保存完成标记识别、严格 JSON 解析和规范指纹比较所需的有限状态，不长期保存已决定丢弃的大段正文。
- [ ] 运行绿灯测试：
