...
describe('generationSession generation-stream/v1 状态机', () => {
  ...
  it.each([
    ...
    ['控制帧多余字段', { ...trustedDisplay(1, '1', 'tool-1', 'REQUESTED', '工具').data, extra: true }],
  ])('%s 被严格拒绝', async (name, data) => {
    const eventName = name === '控制帧多余字段' ? 'trusted-tool-display' : undefined
  ...
  it('上下文压缩、未完成工具链、工具恢复和普通提示优先级固定', () => {
    expect(getGenerationStatusText('compressing', 'recovering', 'recovering', '普通')).toBe(
      '正在压缩上下文，请稍候…',
    )
    expect(getGenerationStatusText('idle', 'recovering', 'recovering', '普通')).toBe(
      '正在继续未完成的构建流程，请稍候…',
    )
    expect(getGenerationStatusText('idle', 'idle', 'recovering', '普通')).toBe(
      '正在校正工具调用，请稍候…',
    )
  })
